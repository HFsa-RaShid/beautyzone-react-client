  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          { title: "Total Order", value: `${orders.length}`, icon: "📦" },
          { title: "Processing", value: "12", icon: "⏳" },
          { title: "Pending Payments", value: "12", icon: "⚠️" },
          { title: "Revenue", value: "$91,526", icon: "📈" },
        ].map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-2xl border border-gray-300 shadow-sm">
      ...
      <div className="border border-gray-300 rounded-xl p-6 mb-4">
        ...
        <div className="w-1/2 flex justify-between gap-2">
          <select className="border border-gray-300 p-2 rounded-lg text-sm w-[50%]">
            <option>All</option>
            <option>Paid</option>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b text-gray-400 text-xs uppercase">
            <tr>
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order._id} className=" hover:bg-gray-50 text-sm">
                ...
                <td className="p-5">{order.customer?.email || "Guest"}</td>
                <td className="p-5">
        ...
        <div className="flex justify-between items-center p-5 border-t border-gray-200">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-1 border rounded text-xs disabled:opacity-50"
            >
              Prev
            </button>
            <button
