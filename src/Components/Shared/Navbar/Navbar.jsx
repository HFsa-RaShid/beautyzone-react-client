            ...
            >
              ...
              {shopOpen && (
                <div className="absolute top-full left-0 w-48 bg-white shadow-xl border border-gray-50 py-3 animate-in fade-in slide-in-from-top-2">
                  <p className="px-4 py-2 hover:bg-pink-50 hover:text-[#b06d72] transition cursor-pointer">Serums</p>
                  <p className="px-4 py-2 hover:bg-pink-50 hover:text-[#b06d72] transition cursor-pointer">Cleansers</p>
                  <p className="px-4 py-2 hover:bg-pink-50 hover:text-[#b06d72] transition cursor-pointer">Moisturizers</p>
                </div>
              )}
            </li>
            <li className="hover:text-[#b06d72] cursor-pointer transition">Collections</li>
            <NavLink to="/aboutUs">
                <li className="hover:text-[#b06d72] cursor-pointer transition">About</li>
            </NavLink>
            <NavLink to="/contactUs">
                <li className="hover:text-[#b06d72] cursor-pointer transition">Contact</li>
            </NavLink>
          </ul>
        </div>
                ...
                <div className="absolute top-full right-0 w-44 bg-white shadow-[0_10px_30px_rgba(0,0,0,0.1)] border border-gray-100 py-3 z-50 animate-in fade-in slide-in-from-top-2">
                  {user ? (
                    ...
                    <>
                      ...
                      <p onClick={() => navigate("/profile")} className="px-4 py-2 text-[11px] font-bold uppercase tracking-wider text-gray-700 hover:bg-gray-50 hover:text-[#b06d72] cursor-pointer transition">
                        My Account
                      </p>
                      <p onClick={() => navigate("/orders")} className="px-4 py-2 text-[11px] font-bold uppercase tracking-wider text-gray-700 hover:bg-gray-50 hover:text-[#b06d72] cursor-pointer transition">
                        Order History
                      </p>
                      <button 
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-gray-100 shadow-lg p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3 pt-4 border-t">
            {user ? (
               <>
                <p className="text-[10px] font-bold uppercase text-gray-400">Logged in as {user.name}</p>
                <button onClick={handleSignOut} className="bg-black text-white px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest">
                  Log Out
                </button>
               </>
            ) : (
              <div className="flex flex-col gap-2">
                <button onClick={() => { navigate("/signup"); setMobileMenuOpen(false); }} className="bg-black text-white px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest">
                  Sign Up
                </button>
                <button onClick={() => { navigate("/signin"); setMobileMenuOpen(false); }} className="border border-black text-black px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest">
                  Sign In
                </button>
              </div>
            )}
